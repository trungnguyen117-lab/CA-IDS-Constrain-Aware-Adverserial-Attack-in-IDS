"""Evaluate models on DAE-purified adversarial samples.

Compares: No Defense | AT | DAE | AT + DAE.

Usage:
    python defense/dae/evaluate_dae.py --target all --device cpu
    python defense/dae/evaluate_dae.py --target lstm resdnn --at both
"""
...
def _find_purified_csvs(target, purified_root):
    """Find DAE-purified adversarial CSVs for a target."""
    results = {}
...
def evaluate_target(target, purified_root, at=False, device="cpu"):
    """Evaluate a target on clean + DAE-purified adversarial data."""
    tag = f"{target.upper()} {'(AT+DAE)' if at else '(DAE)'}"
    logger.info(f"{'='*60}")
    ...
    # Check for purified clean test (to measure DAE impact on clean data)
    ...
    # DAE-purified adversarial predictions
    adv_files = _find_purified_csvs(target, purified_root)
    if not adv_files:
        logger.info("No DAE-purified adversarial samples found.")
        return results
...
def _find_adv_csvs_inline(target):
    """Find raw adversarial CSVs for baseline evaluation."""
    results = {}
    if target in DL_TARGETS:
...
def _print_four_way_comparison(baseline, at, dae, at_dae):
    """Print 4-way comparison table: No Defense | AT | DAE | AT+DAE."""
    ...
    all_attacks = []
    for results_dict in [baseline, at, dae, at_dae]:
    ...
    for target in targets:
        t = Table(
            title=f"{target.upper()} — DAE Defense Comparison",
        ...
        t.add_column("DAE", justify="right", min_width=12)
        t.add_column("AT + DAE", justify="right", min_width=12)
        ...
        # Clean accuracy
        row = ["Acc (clean)"]
        for rd in [baseline, at, dae, at_dae]:
            row.append(_fmt(rd.get(target, {}).get("clean", {}).get("acc")))
        t.add_row(*row)

        # Clean purified accuracy (only for DAE columns)
        if dae.get(target, {}).get("clean_purified"):
            row = ["Acc (clean purified)"]
            row.append("-")  # baseline
            row.append("-")  # AT
            row.append(_fmt(dae[target].get("clean_purified", {}).get("acc")))
            row.append(_fmt(at_dae.get(target, {}).get("clean_purified", {}).get("acc")))
            t.add_row(*row)
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate DAE defense")
    ...
    purified_root = os.path.join(_HERE, "purified")

    if not os.path.isdir(purified_root):
        logger.error(f"Purified directory not found: {purified_root}")
        logger.error("Run purify_dae.py first to generate purified CSVs.")
        sys.exit(1)
    ...
    for target in targets:
        ...
        # 3. DAE (baseline model on DAE-purified adv)
        dae_results[target] = evaluate_target(
            target, purified_root, at=False, device=args.device)

        # 4. AT + DAE (AT model on DAE-purified adv)
        if args.at in ("true", "both"):
            try:
                at_dae_results[target] = evaluate_target(
                    target, purified_root, at=True, device=args.device)
            except Exception as e:
                logger.warning(f"AT+DAE not available for {target}: {e}")
                at_dae_results[target] = {}

    # Print comparison
    _print_four_way_comparison(baseline_results, at_results, dae_results, at_dae_results)

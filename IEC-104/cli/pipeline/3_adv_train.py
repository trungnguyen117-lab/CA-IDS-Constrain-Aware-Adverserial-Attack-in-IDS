...
Follows notebook pattern:
  - DL (LSTM, ResDNN): clean + self-attack WB adv (weighted sampling) → retrain
  - ML (CAT, RF): clean + multi-source transfer adv (resdnn_sc) + BB adv → retrain
"""
...
from utils.constants import (
    ...
    AT_WEIGHTS_LSTM, AT_WEIGHTS_RESDNN, AT_WEIGHTS_ML,
...
from utils.paths import get_path, adv_train_dir, model_path, set_version, model_stem
...
def _load_adv_csv(path):
    """Load a single adv CSV, return DataFrame or None."""
...
def _weighted_sample_parts(sources, n_budget, rng):
    """Sample from (DataFrame, weight) list according to weights.

    Follows exact notebook pattern:
      n_sample = int(budget * weight / total_weight)
      idx = rng.choice(len(df), size=n_sample, replace=need_replace)
    """
    ...
    for name, df, weight in sources:
        ...
        classes = np.unique(y)
        per_class = max(1, n_sample // len(classes))
        strat_idx = []
        for c in classes:
            c_idx = np.where(y == c)[0]
            n_c = min(per_class, len(c_idx))
            strat_idx.append(rng.choice(c_idx, size=n_c, replace=len(c_idx) < n_c))
        idx = np.concatenate(strat_idx)
        rng.shuffle(idx)
...
def _assemble_clean_and_adv(feature_names, X_clean, y_clean, parts_X, parts_y):
    """Merge clean + adv, dedup, return DataFrame."""
    if not parts_X:
    ...
    # Balance classes: downsample to min class count
    rng_bal = np.random.RandomState(42)
    counts = df_tmp["Label"].value_counts()
    min_count = counts.min()
    balanced_parts = []
    for c in sorted(counts.index):
        df_c = df_tmp[df_tmp["Label"] == c]
        if len(df_c) > min_count:
            df_c = df_c.sample(n=min_count, random_state=rng_bal)
        balanced_parts.append(df_c)
    df_tmp = pd.concat(balanced_parts).reset_index(drop=True)

    logger.info(f"AT train: {df_tmp.shape} (balanced: {min_count} per class)")
    return df_tmp


def assemble_dl_at_data(target):
    """Assemble AT data for DL model: clean + self-attack WB adv (weighted).

    Notebook pattern (defense_LSTM.ipynb Cell 24):
      budget = n_clean / CLEAN_ADV_RATIO
      For each WB attack: sample proportional to weight
    """
    ...
    sources = []
    at_weights = AT_WEIGHTS_LSTM if target == "lstm" else AT_WEIGHTS_RESDNN
...
def assemble_ml_at_data(target):
    """Assemble AT data for ML: clean + multi-source transfer adv + BB adv.

    Notebook pattern (defense_CAT_resdnn_sc.ipynb Cell 9):
      Transfer from resdnn_sc (weighted)
      Direct BB from {target}/ (if available)
    """
    _, X_clean, y_clean, feature_names = load_dataset("train_tvae")
    n_clean = len(X_clean)
    logger.info(f"Clean train: {n_clean} samples")
    ...
    # Transfer from resdnn_sc (WB + BB): all attacks in AT_WEIGHTS_ML
    for src in AT_TRANSFER_SOURCES.get(target, []):
        src_dir = adv_train_dir(src)
        src_model = src.replace("_sc", "")  # "resdnn"
    ...
    # Direct BB: adv_training/{target}/{target}_{atk}_train_adv.csv
...
def retrain_model(target, df_merged, device="cpu"):
    """Retrain model on merged AT data and save via model registry."""
    ...
    # Special AT config for CatBoost: no eval_set, no early stop
    at_cfg = None
    if target == "cat":
        at_cfg = dict(
            iterations=5000, depth=8, learning_rate=0.05,
            l2_leaf_reg=3, od_type=None, od_wait=None,
        )
    elif target in DL_TARGETS:
        # Fit InputNorm on merged clean+adv data (matches APELID/notebook pattern)
        at_cfg = {
            "patience": 40,
            "max_epochs": 200,
            "warmup_epochs": 8,
        }
        logger.info(f"InputNorm will fit on merged AT data for {target.upper()}")

    # Train via registry
    m = get_model(target)
    # For CAT AT: pass None as X_val to skip eval_set
    ...
    m.save(os.path.join(models_at, f"{model_stem(target, at=True)}{ext}"))
...
def main():
    ...
    parser.add_argument("--model", "-m", nargs="+", required=True,
                        ...
                        help="Models to retrain with AT")

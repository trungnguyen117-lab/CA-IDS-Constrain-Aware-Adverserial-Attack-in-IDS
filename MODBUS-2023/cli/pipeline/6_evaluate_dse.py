"""Evaluate models with DSE query detection defense.

Mirrors 2_evaluate.py but adds DSE-based detection metrics:
- Detection rate: % of adversarial samples flagged by DSE
- Adjusted ASR: ASR after DSE blocks detected queries
- FPR: false positive rate on clean test data
"""
...
from utils.dse_detector import DSEDetector
...
def _find_adv_csvs(target):
    """Find adversarial eval CSVs for a target (same logic as 2_evaluate.py)."""
    results = {}
...
def evaluate_with_dse(target, detector, at=False, device="cpu",
                      defense=None, sigma=0.01, clip_values=None):
    """Evaluate a target model with DSE query detection."""
    logger.info(f"{'='*60}")
    tag = f"{target.upper()} {'(AT)' if at else '(baseline)'} +DSE"
    if defense:
    ...
    clean_metrics["dse_fpr"] = fpr_clean
    ...
    for name, path in adv_files.items():
        try:
            ...
            # DSE detection (pass X_test as clean originals)
            det_rate = detector.compute_detection_rate(X_adv, X_clean=X_test)
            adj_asr = detector.adjusted_asr(
                y_test, y_clean, y_adv_pred, X_adv, X_clean=X_test,
            )
            metrics["dse_det_rate"] = det_rate
            metrics["dse_adj_asr"] = adj_asr
            logger.info(
                f"{'':>25s}  DSE: det={det_rate:.2f}%, "
...
def _print_dse_summary(all_results, title="DSE EVALUATION SUMMARY"):
    """Print summary table with DSE metrics."""
    ...
    row = ["FPR (clean, DSE)"]
    for tgt in targets:
        val = all_results[tgt].get("clean", {}).get("dse_fpr")
        row.append(f"{val:.2f}%" if val is not None else "-")
    t.add_row(*row)
    ...
    for atk in all_attacks:
        ...
        row = [f"ASR+DSE ({atk})"]
        for tgt in targets:
            val = all_results[tgt].get(atk, {}).get("dse_adj_asr")
            row.append(f"{val:.2f}%" if val is not None else "-")
        ...
        row = [f"Det% ({atk})"]
        for tgt in targets:
            val = all_results[tgt].get(atk, {}).get("dse_det_rate")
            row.append(f"{val:.2f}%" if val is not None else "-")
        t.add_row(*row)
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate models with DSE query detection defense"
    )
    ...
    parser.add_argument("--dse-model", default=None,
                        help="Path to DSE encoder checkpoint")
    ...
    # Load DSE model
    dse_path = args.dse_model or os.path.join(
        get_path("models"), "dse_encoder.pth"
    )
    dse = DSEModel.load(dse_path, device=args.device)

    # Load DSE config for detection params
    dse_cfg_path = training_config_path("dse")
    if os.path.isfile(dse_cfg_path):
        with open(dse_cfg_path) as f:
            dse_cfg = yaml.safe_load(f) or {}
    else:
        dse_cfg = {}

    # Build baseline from normal traffic (training data)
    _, X_normal, _, _ = load_dataset("train_tvae")
    detector = DSEDetector(dse, X_normal, cfg=dse_cfg)

    # Optional calibration
    if args.calibrate:
        _, X_test, _, feature_names = load_dataset("test")
        # Use first available adversarial CSV for calibration
    ...
    for target in targets:
        all_results[target] = evaluate_with_dse(
    ...
    _print_dse_summary(
        all_results,
        title=f"DSE EVALUATION {'(AT)' if use_at else '(baseline)'}",
    )

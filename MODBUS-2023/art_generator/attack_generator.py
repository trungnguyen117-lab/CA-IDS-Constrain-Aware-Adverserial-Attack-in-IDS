...
class BatchedAttackGenerator(AttackGenerator):
    ...
    def generate(self, x, y, input_metadata, mutate_indices=[], **kwargs):
        if x is None or x.size == 0:
            cols = input_metadata.get("feature_names", []) + [
                input_metadata.get("label_column", "label")]
        ...
        if batch_size is None or batch_size <= 0 or batch_size >= n_samples:
            ...
            logger.info(
                f"[+] Generating adversarial samples with {self.NAME} "
            ...
            for start in range(0, n_samples, batch_size):
                ...
                xb_adv = self._try_batch(
                    xb, maskb, start, end, max_retries, timeout_seconds)
                ...
                if verbose_every and verbose_every > 0 and (
                    ...
                    logger.info(
                        f"[{self.NAME}] Progress: {processed}/{total_batches}, "
                        f"failed={failed}")
    ...
    def _try_batch(self, xb, maskb, start, end, max_retries, timeout_seconds):
        for attempt in range(1, max_retries + 1):
            try:
                ...
                logger.warning(
                    f"Batch {start}:{end} attempt {attempt}/{max_retries} "
                    f"failed: {e}")
        return None
